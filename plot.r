...
print(summary(model))
df$deltaPSIc = round(model$residuals, digits=2) + 0.5*p.cutoff
...
p = ggplot(df2, aes(x=log10sjcount, y=deltaPSIc, color=(p.adj<p.cutoff))) + geom_point(size=1, alpha=0.5)
p = p + geom_abline(slope=0,lty="dashed") + geom_label_repel(size=3, data=df3, aes(label = gene), min.segment.length = 0, force=10)
