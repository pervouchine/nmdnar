...
control1 = read.delim(paste("data/crispr/A07/",args[3],".A07.tsv", sep=""), col.names=c('id','inc','exc'))
control2 = read.delim(paste("data/crispr/A07/",args[4],".A07.tsv", sep=""), col.names=c('id','inc','exc'))
control = compute_psi(pool_biorep(control1, control2))

target1 = read.delim(paste("data/crispr/A07/",args[1],".A07.tsv", sep=""), col.names=c('id','inc','exc'))
target2 = read.delim(paste("data/crispr/A07/",args[2],".A07.tsv", sep=""), col.names=c('id','inc','exc'))
...
df$KDFC = 0 #round((A$inc.y/A$inc.x)/(B$inc.y/B$inc.x),digits=2)
